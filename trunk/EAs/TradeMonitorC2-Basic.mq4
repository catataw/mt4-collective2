//+------------------------------------------------------------------+
//|                                                 TradeMonitor.mq4 |
//|                     Copyright © 2011, OpenThinking Software, LLC |
//|                              http://www.openthinkingsoftware.com |
//+------------------------------------------------------------------+
#property copyright "Copyright © 2011, OpenThinking Software, LLC"
...
extern bool RoundDownLots = true;
...
{
    double C2LotSize = NormalizeDouble((volume * C2LotMultiplier),2);
    string c2sigid = "0";
    double stoplossValue = 0.0;
    double takeprofitValue = 0.0;
    string edittedSymbol = StringSubstr(symbol,0,3) + "/" + StringSubstr(symbol,3,3);
    
    if (C2LotSize != NormalizeDouble(C2LotSize,0))
    {
        if (RoundDownLots == true)
        {
         C2LotSize = MathFloor(C2LotSize);
        }
        else
        {
...
void SendOrderModify(int orderid, double price, double stoploss, double takeprofit, double origstoploss, double origtakeprofit, double origprice)
{
    ...
    while (tries < 5)
    {
        ...
        int statusCode[1] = {0};
        string statusText =   "88888888888888888888888888888888888888888888888888888888888888888888888888888888888888888";
        PrintMessage("OrderModify not available in this version.  Please see website for more details.");
